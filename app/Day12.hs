...
updateState (position@(posN, posE), direction@(dirN, dirE)) (instructionType, arg)
...
solution input = abs lastPosN + abs lastPosE
    where
        instructions = fromList @(Vector _) . map parseLine . lines $ input
        ((lastPosN, lastPosE), _) = foldl updateState ((0, 0), (0, 1)) instructions
